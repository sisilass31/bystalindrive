...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!token) {
    ...
    try {
      const res = await fetchWithLoader(`${API_URL}/check-token`, {
        method: "POST",
      ...
      showModal(
        "Erreur serveur lors de la vérification du lien.",
        "error",
        () => {
          window.location.href = "/pages/login.html";
        }
  ...
  if (userToken) {
    try {
      const res = await fetchWithLoader(
        `${API_URL.replace("/api/users", "/api/users/me")}`,
        {
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    if (!password || !confirm)
      return showModal("Veuillez remplir tous les champs", "error");
    if (password !== confirm)
      return showModal("Les mots de passe ne correspondent pas", "error");
    if (!Object.values(checkPassword(password)).every(Boolean))
      return showModal("Mot de passe non conforme", "error");

    try {
      const res = await fetchWithLoader(`${API_URL}/set-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        showModal(
          data.message || "Erreur lors de l’activation du compte",
          "error"
        );
      }
    } catch {
